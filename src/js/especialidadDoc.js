...
addOptions("Especial", especialidadesDoc);
function addOptions(domElement, especialidadesDoc) {
    var select = document.getElementsByName(domElement)[0];
    for (value in especialidadesDoc) {
     var option = document.createElement("option");
     option.text = especialidadesDoc[value];
     select.add(option);
    }
}

//let opt = document.createElement("option");
//let imprimir = opt.text(`<option value="` + optiones +`"` + `>` + optiones + `</option>`);
//for (let index = 0; index < especialidadesDoc.length; index++) {
//    optiones += especialidadesDoc[index];
//    document.querySelector("#Especial").innerHTML = imprimir;
//} 
